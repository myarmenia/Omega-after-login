...
import arrowUp from './images/helpers/arrowUP.png';
import arrowUpBlack from './images/helpers/arrowUP_black.png';
...
const arrowUpStyle = {
  ...
  linear-gradient(rgba(109, 228, 24, 0.31) 5%, rgb(46, 169, 93) 100%)`
}
...
class Socketclient extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            /* multi-mode */
            endpoint: "https://omegacoding.com:55263", //"https://omegacoding.com/", //endpoint: "http://localhost:7000/",
            ...
            showDemo: true,
            lng: this.props.lang,
            charCase: false, // false -> lower; true -> upper
        };
    }
...
changeAllSpaces=(flag)=>{
  let keyBtns = document.getElementsByName("space");

  if (flag === 'disable') {
    for (let i = 0; i < keyBtns.length; i++) {
      keyBtns[i].setAttribute('disabled','true');
    }
  }

  if (flag === 'flipGreen') {
    for (let i = 0; i < keyBtns.length; i++) {
      keyBtns[i].style.background ='#87fd87'; //'green';
      keyBtns[i].classList.add("animated","flip");
    }
    setTimeout(function() {
      for (let i = 0; i < keyBtns.length; i++) {
        keyBtns[i].classList.remove("animated","flip");
      }
    }, 300);
  }
  ...
  if (flag ==='fillRed') {
    for (let i = 0; i < keyBtns.length; i++) {
      keyBtns[i].style.background = 'rgb(251, 218, 162)';//'orange';
      keyBtns[i].style.color = 'red';
    }
  }
    ...
    componentDidMount() {
        ...
        let wrap_search_btns = document.getElementsByClassName('wrap-socket-search-btns')[0];

        let bet_socket_search = document.getElementsByClassName('bet-socket-search')[0];


        let loading_time_20 = 0;
        let loading_time_points = '.';
        ...
        socket.on('connect_error', ()=> {
          ...
          messages_from_server.innerHTML = 'There is a problem with server connection. Please, try later or contact with Admin!';
          see_my_socket_id.innerHTML = '';
        })
        ...
        socket.on("connect", ()=> {
            ...
            socket.emit("adduser", this.state.Uname, this.props.gameId, this.props.userProfile.firstname, this.props.lock, this.props.userProfile.userImg);
            ...
            //let see_my_socket_id = document.getElementsByClassName('see-my-socket-id')[0];
            console.log('see_my_socket_id-> ',see_my_socket_id);
            see_my_socket_id.innerHTML = `Hi ${this.props.userProfile.firstname}!`;
        ...
        socket.on("otherLevelFreeRooms", (choiceData)=>{
          ...
          if (choiceRooms.length) {
                ...
                return(
                  ...
                  <li key={freeOne.name} className='free-room-elem' data-conn-id={freeOne.id}>
                    ...
                    <span className="span-space">
                      <img src={"https://omegacoding.com/android_test/tmp/"+freeOne.image}  width="40px"/>
                    </span>
            ...
            // let loading_20_id = setInterval(()=> {
            //   socket.emit("searchRoomEvent", client_id);
            // } ,1000);
            loading_time_20 += 1;
            loading_time_points += '.';

            if (loading_time_20 < 3) {

              messages_from_server.innerHTML = 'Searching for opponent '+ loading_time_points + ' ' + loading_time_20;
              messages_from_server.classList.add("animated","slideInDown");
              this.setState({ freeRoomsItems : null });
            }else{
              let unfo_message = 'Unfortunately at this moment there are no players available. Press the button to play single.';


              messages_from_server.innerHTML = unfo_message; // animated slideInUp
              messages_from_server.classList.add("animated","slideInUp");
              wrap_search_btns.classList.remove("hide_block"); // wrap_search_btns
              wrap_search_btns.classList.add("show_block"); // wrap_search_btns

              bet_socket_search.classList.remove("animated","flipInX");
        ...
        this.handleSearch=(e)=> { // search empty room
            ...
            loading_time_20 = 0;
            loading_time_points = '.';
            bet_socket_search.classList.add("animated","flipInX");
            // messages_from_server.innerHTML = 'Searching for opponent .' + loading_time_points;
            messages_from_server.innerHTML = '';
            messages_from_server.classList.remove('slideInDown', 'slideInUp');

            wrap_search_btns.classList.remove("show_block"); // wrap_search_btns
            wrap_search_btns.classList.add("hide_block"); // slideInDown slideInUp

            if (this.state.search_interval_id !== 0) {
              clearInterval(this.state.search_interval_id);
            }
            let search_interval_id = setInterval(()=> {
              socket.emit("searchRoomEvent", client_id);
        ...
        this.playSingle =()=> {
          console.log('play-single clicked');
          this.props.playSingle(false);
        }
                  ...
                  if(drown === true) {
                        ...
                        if(countRepeats === 1) {
                            ...
                            if (n !== " ") {
                              ...
                              this.changeAllSpaces('flipGreen');
                            }
...
/* gameEnd */
  ...
  toggleLanguage=()=>{
    if(this.state.lng === 'eng') {
      this.setState({ lng: 'arm'});
    } else {
      this.setState({ lng: 'eng' });
    }
  ...
  toggleCase=()=>{
    this.setState({
      charCase: !this.state.charCase,
    });
    let caseSwitcher = document.getElementsByClassName('bet-change-case')[0];

    if (!this.state.charCase) {
      // is upperCase
      caseSwitcher.style.backgroundColor = '#2ea95d';
      caseSwitcher.style.backgroundImage = `url(${arrowUpBlack}),
      linear-gradient(rgba(109, 228, 24, 0.31) 5%, rgb(46, 169, 93) 100%)`
    }else{
      // is toLowerCase
      caseSwitcher.style.backgroundColor = '#6de4184f';
      caseSwitcher.style.backgroundImage = `url(${arrowUp}),
      linear-gradient(rgba(109, 228, 24, 0.31) 5%, rgb(46, 169, 93) 100%)`
    }
  }
  ...
  render() {
    ...
    let keyLng = this.state.lng;
    let charCase = this.state.charCase;
    ...
    // show-hide lang-switcher by lang-props
    let showLngBtn = '';
    if (this.props.lang === 'arm') {
      showLngBtn = {display: 'block'};
    }else {
      showLngBtn = {display: 'none'};
    }

    // switching special and alphabetical keyboards
    let specBtnText = '?123.';
    let showKeyboard = {display: 'inline-block'};
    let showSpecials = {display: 'none'};
    if(this.state.isOpenSpecials){
    ...
    return (
        ...
        {(seeEachOther && Uname)?
          ...
          <FetchNver
              ...
              {(!robImages || !betArray)?
              <span className="bet-loader" >Loading ...</span>:

              <div className="multi-window" style={gameBackStyle}>
                <div className="test-place">
                  {/*sendMsgBtn ? <button onClick= {this.handleMessage}>test Message</button> : null this.pauseTimer */}
                </div>
                ...
                <div className="bet-multipanel-wrap">
                    ...
                    <div className="name-and-progress-wrap">
                      ...
                      <div className="bet-this-that-user left-user border-blue-off">
                        <span className="this-that-user-name">
                          <span className="this-that-user-img-wrap">
                            <img src={"https://omegacoding.com/android_test/tmp/"+this.state.thisUser.image} className="this-that-user-img"/>
                          </span>
                          <span className="user-flag-wrap">
                            <img className="user-flag" src="https://omegacoding.com/android_test/images/flags/am.png" />
                          </span>
                          <span className="user-name-wrap">
                            {this.state.thisUser.realyName}
                          </span>
                        </span>
                      ...
                      <div className="bet-this-that-user right-user border-green-off">
                        <span className="this-that-user-name">
                          <span className="this-that-user-img-wrap">
                            <img src={"https://omegacoding.com/android_test/tmp/"+this.state.thatUser.image} className="this-that-user-img"/>
                          </span>
                          <span className="user-flag-wrap">
                            <img className="user-flag" src="https://omegacoding.com/android_test/images/flags/am.png" />
                          </span>
                          <span className="user-name-wrap">
                            {this.state.thatUser.realyName}
                          </span>
                        ...
                        <div className="bet-rounds-timer-helper-wr">
                          <div className="bet-rounds-wr">
                            ...
                            <div className="bet-roud">
                              <span className="round-title">{trans.buttons.rounds}</span>
                ...
                <div className="game-foot">
                  ...
                  <div className="bet-more-btn-wrap">
                    <button className="bet-special-simbols" onClick={this.toggleSpecials}>{specBtnText}</button><br/>
                    <button className="bet-exit-game"       onClick={this.goToLevelList}>{trans.buttons.exit}</button><br/>

                  <span style={showLngBtn}>
                    <button onClick={ this.toggleLanguage} className="bet-exit-game">{ lngBtnText }</button><br/>
                  </span>

                  <button onClick={this.toggleCase} className="bet-exit-game bet-change-case" style={arrowUpStyle}></button><br/>
                  {/*
                  ...
                  <div className="bet-keyboard" onClick={this.handleClick} style={showKeyboard}>
                    <Keyboard lng={keyLng} charCase= {charCase} />
                  </div>
        ...
        :
        <div className="bet-socket-search-wrap">
        <div className = "bet-socket-search animated flipInX">
          <div className = "bet-socket-search-bg">
              <p className="see-my-socket-id animated bounceInUp"></p>
              <p className="messages-from-server"></p>

              <div className="wrap-socket-search-btns hide_block">
                <button className="bet-search-btn" onClick={this.playSingle}>Play</button>
                <button className="bet-search-btn" onClick= {this.handleSearch}>Search Again</button>
              </div>
