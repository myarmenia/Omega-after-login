    ...
    let  re = /^(Moz|(w|W)ebkit|O|ms)(?=[A-Z])/;

    // Establish vendor prefix and CSS 3D support
    let  vendor = (function() { for ( let  p in el.style ) if( re.test(p) ) return p.match(re)[0]; })() || '';
    let  canRun = vendor + 'Perspective' in el.style;
    let  prefix = '-' + vendor.toLowerCase() + '-';
